...
5. **No universal last-resort.** When the declared chain is exhausted, the call fails — the runtime never falls through to a hidden provider the caller didn't name. Lane attribution stays honest: a Builder pinned to `ollama/qwen3.5:9b` either succeeds on that model or fails as that model. If you want a safety net, declare it in the chain.

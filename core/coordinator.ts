...
function detectAmbiguity(input: string): boolean {
  ...
  const hasFilePath = /[A-Za-z0-9_-]+\/[A-Za-z0-9._/-]+/.test(trimmed) ||
    ...
    /[A-Za-z0-9_-]+\.(?:py|pyi|rs|go|java|kt|swift|c|cc|cpp|h|hpp|cs|rb|php|scala|lua|sh|bash|gd|tscn|tres|gdshader|vue|svelte|css|scss|sass|less|html|json|yaml|yml|toml|md|txt)/.test(trimmed);

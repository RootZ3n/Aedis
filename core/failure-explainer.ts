...
export function explainFailure(receipt: RunReceipt): FailureExplanation {
  ...
  return {
    ...
    stage: "unknown",
    rootCause:
      "The run did not reach a verified success state, but no specific blocker was captured on the receipt. This typically means the provider returned an empty response, a worker produced no usable diff, or a gate failed silently — in any case, no diff was applied.",
    suggestedFix:
